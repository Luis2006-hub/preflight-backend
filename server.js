...
app.get("/", (req, res) => {
  res.json({ status: "Preflight Pro v7 (Tekkrom) activo" });
});
...
async function startAnalysis() {
  ...
  try {
    ...
    try {
      const vista = await makeAnalysisPreview();
...
async function makeAnalysisPreview() {
  ...
  if ((curExt === 'ai' || curExt === 'eps') && window._pdfDoc) {
    try {
...
async function handleAnalysis(req, res) {
  try {
    ...
    if (!archivoFile) return res.status(400).json({ error: "No se recibió archivo" });
    const { originalname, mimetype, buffer, size } = archivoFile;
...
app.listen(PORT, () => console.log("Preflight Pro v7 (Tekkrom) puerto " + PORT));
